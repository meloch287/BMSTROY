...
import { LayoutDashboard, FileText, Settings, Image as ImageIcon, HelpCircle, DollarSign, Calculator, MessageCircle, FileCheck, Sparkles, UserCheck, Layers, Users, Film, Calendar, BarChart } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#020617] flex">
      <aside className="w-64 bg-[#0F172A] border-r border-white/5 flex flex-col fixed h-full z-50">
        ...
        <nav className="flex-1 p-4 space-y-2 overflow-y-auto no-scrollbar text-sm">
            ...
            <Link href="/admin/video" className="flex items-center gap-3 text-gray-400 hover:text-white hover:bg-white/5 p-3 rounded-xl transition-colors"><Film size={18}/> Видео</Link>
            ...
            <Link href="/admin/analytics" className="flex items-center gap-3 text-gray-400 hover:text-white hover:bg-white/5 p-3 rounded-xl transition-colors"><BarChart size={18}/> Аналитика</Link>
            <Link href="/admin/booking-slots" className="flex items-center gap-3 text-gray-400 hover:text-white hover:bg-white/5 p-3 rounded-xl transition-colors"><Calendar size={18}/> Слоты замеров</Link>
